...
$css_class .= (!empty($item_align)) ? ' text-' . $item_align : '';
...
<?php if ($box_style == 'style_1') :
    ...
    if(empty($content)) $css_class .= ' empty_text';
    ?>
	...
	<?php if ($style == 'icon_left') { ?>
		...
		<?php endif; ?>
		<?php if ($icon) { ?>
            <div class="icon<?php echo crypterio_sanitize_text_field($icon_class); ?>"<?php echo crypterio_sanitize_text_field($icon_style); ?>><i
                        style="font-size:<?php echo esc_attr($icon_size); ?>px; <?php if ($icon_color_custom) { ?>color: <?php echo esc_attr($icon_color_custom); ?>;<?php } ?> <?php if ($icon_bg_color_custom) { ?>background-color: <?php echo esc_attr($icon_bg_color_custom); ?>;<?php } ?>"
                        class="<?php echo esc_attr($icon); ?>"></i></div>
		<?php } ?>
        <div class="icon_text" data-readmore="<?php echo esc_attr($link_title); ?>">
			<?php if ($title) { ?>
                <h5<?php echo crypterio_sanitize_text_field($title_style); ?><?php echo crypterio_sanitize_text_field($title_class); ?>><?php echo wp_kses($title, array('br' => array())); ?></h5>
			<?php } ?>
	...
	<?php } else { ?>
        ...
        <div class="icon_text" data-readmore="<?php echo esc_attr($link_title); ?>">
			<?php if ($title) { ?>
                <h4<?php echo crypterio_sanitize_text_field($title_style); ?><?php echo crypterio_sanitize_text_field($title_class); ?>><?php echo wp_kses($title, array('br' => array())); ?></h4>
			<?php } ?>
			<?php echo wpb_js_remove_wpautop($content, true); ?>
        </div>
		<?php if (!empty($link['url'])): ?>
...
<?php elseif ($box_style == 'style_2') : ?>
    ...
    <div class="icon_box_inner">
		<?php if ($icon) { ?>
            <div class="icon<?php echo crypterio_sanitize_text_field($icon_class); ?>"<?php echo crypterio_sanitize_text_field($icon_style); ?>><i
                        style="font-size:<?php echo esc_attr($icon_size); ?>px; <?php if ($icon_color_custom) { ?>color: <?php echo esc_attr($icon_color_custom); ?>;<?php } ?>"
                        class="<?php echo esc_attr($icon); ?>"></i></div>
		<?php } ?>
		<?php if ($title) { ?>
            <h5<?php echo crypterio_sanitize_text_field($title_style); ?><?php echo crypterio_sanitize_text_field($title_class); ?>><?php echo wp_kses($title, array('br' => array())); ?></h5>
...
<?php elseif ($box_style == 'style_3') : ?>
    ...
    <div class="icon_box_inner">
		<?php if ($icon) { ?>
            <div class="icon<?php echo crypterio_sanitize_text_field($icon_class); ?>"<?php echo crypterio_sanitize_text_field($icon_style); ?>><i
                        style="font-size:<?php echo esc_attr($icon_size); ?>px; <?php if ($icon_color_custom) { ?>color: <?php echo esc_attr($icon_color_custom); ?>;<?php } ?> <?php if ($icon_line_height) { ?>line-height: <?php echo esc_attr( $icon_line_height ); ?>px;<?php } ?> <?php if ($icon_border_color_custom) { ?>border-color: <?php echo esc_attr($icon_border_color_custom); ?><?php } ?>"
                        class="<?php echo esc_attr($icon); ?>"></i></div>
		<?php } ?>
		<?php if ($content) { ?>
            <div class="icon_text">
