...
class STM_PostType {
			...
			switch ( $field['type'] ) {
				...
				case 'image_preview':
					...
					if ($meta) {
						...
						$image = $image[0];
						$full = wp_get_attachment_image_src($meta, 'full');
						$full = $full[0];
					}
					if( empty($image) ){
						$image = $default_image;
					...
					echo '
						<div class="stm_metabox_image stm_metabox_image__preview">
							<input name="'. $key .'" type="hidden" class="custom_upload_image" value="'. $meta .'" />
							<img src="'. $image .'" class="custom_preview_image" alt="" />
							<a target="_blank" href=" ' . $full . '" class="button-primary" type="button" >' . __( 'View File', STM_POST_TYPE ) . '<a/>
						</div>
					';
					break;
			}
	...
	public static function addTaxonomy( $slug, $taxonomyName, $post_type, $args = '', $rewrite_url = '' ) {

		$rewrite_url = (!empty($rewrite_url)) ? $rewrite_url : $slug;
		...
		$defaults = array(
			...
			'query_var'         => true,
			'rewrite'           => array( 'slug' => $rewrite_url )
		);
